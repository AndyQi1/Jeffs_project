...
def estimate_all():
    ...
    try:
        ...
        try:
            ...
            return json.dumps(
                {"success": False, "message": "Feature extraction failed"}
            )
        ...
        return json.dumps(
            {"success": False, "message": f"Calculation failed: {str(e)}"}
        )
...
def receive_bp_data():
    ...
    if "sbp" in form_data:
        try:
            val = float(form_data["sbp"])
        except:
            val = 0.0
        # latest_bp['sbp'] = val # 取决于你的需求，是否要覆盖算法值
    if "dbp" in form_data:
        try:
            val = float(form_data["dbp"])
        except:
            val = 0.0
        # latest_bp['dbp'] = val
    return "OK"
...
def export_data():
    ...
    writer.writerow(
        ["timestamp", "seconds", "s0_raw", "s1_raw", "s0_bpm", "s1_bpm", "SBP", "DBP"]
    )
    ...
    response.headers["Content-Disposition"] = (
        f"attachment; filename=ppg_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    )
